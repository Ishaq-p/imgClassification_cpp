...
std::vector<float> softmax(const std::vector<float>&  logits, int& class_){
    std::vector<float> exp_vals(logits.size());
    float classifiedClass=0.0f;
    class_=0;
...
int mini_main(const std::string& filename1){
    ...
    std::vector<Matrix> img1 = {readPgm(filename, 1000000000)};
    ...
    std::vector<float> fc_output_soft = softmax(fc_output, classified_class);
    std::cout << filename1 << " the number in the img is: " << classified_class;
...
int main(){
    ...
    std::cout << corrected << " the accuracy: "<< float(corrected)/ 100.0f; //float(y.size());
